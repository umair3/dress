...
%It provides us with the vocabulary of three documents; REGISTRATION, DEGREE and TRANSCRIPT. In the coming %sections, we will discuss the vocabulary including the attributes and the possible values for each of %these schema documents that DRESS provide.  
...
Figure~\ref{fig:schema_document} represents partial representation of a schema document.
...
\end{figure}
	...
	\begin{enumerate}  
		...
		P = \{ p1(sellers, seller), p2 (sellers, order, seller) , p3 (seller, sellerID), p4 (seller, sellerName), ... , p10 (sellerID, pattern), p11 (sellerID, use), p12 (sellerName, type), p13 (sellerName, occurs), ...
		...
		\item The third step is the selection of the fault classes. 
		
		\item The fourth step involves writing queries based upon the identified fault classes. 
...
\chapter{Conclusion \& Future Work}\label{ch:Conclusions}
	...
	%\subsection{Search Across Databases} \cite{Search Across Space Databases}
